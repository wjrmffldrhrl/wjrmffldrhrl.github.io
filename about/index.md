---
layout: page
type: about
---

# 조승현

## 👨‍💻WIGO AI Lab Engineer

![https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fwww.notion.so%2Ffd24a04fb827425f933ad6621b242cba&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fwww.notion.so%2Ffd24a04fb827425f933ad6621b242cba&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false)

데이터 처리, 분석, 인공지능 

---
## 👦 Profile

👶 1996년 3월 2일 출생

🏫 광명고등학교 졸업 (2012.03. ~ 2015.02.)

👨‍🎓 성결대학교 재학중 (2015.03 ~ 2021.02.)

---

## 💡 Contact

✉️  wjrmffldrhrl@gmail.com 

✉️  cleanjjsh@naver.com 

📞  010-5261-1586

---

## 🏆 Prize

- 성결대학교 2019학년도 모바일 프로그래밍 경진대회 최우수상
- 서울시 2019 SEOUL 미세먼지 해커톤 대회 우수상
- 성결대학교 파이데이아칼리지 2019학년도 교육용 소프트웨어 경진대회 우수상
- 한국정보산업연합회 2019 한이음 공모전 입선

---

## 🗃 Github


[wjrmffldrhrl - Overview](https://github.com/wjrmffldrhrl)

---

## ⚒Personal & Team Project


- 대학교 프로젝트

### 콘솔 멀티 루미큐브

콘솔 환경 실시간 루미큐브 게임

> 교내 경진대회 출품작 (우수상)

- 기간 : 2018.04.01 ~ 2018.06.25
- 언어 : C
- 역할
    - 팀장
    - TCP/IP Socket 통신 구현
    - 전체적인 게임 플레이 구현
- 관련 기술
    - `C`, `C++`, `TCP/IP Socket`

### [안돼 너 자면 큰일나](https://play.google.com/store/apps/details?id=com.pro.annu)

학생들을 타겟으로 한 졸음 방지 어플리케이션

> 교내 경진대회 출품작 (최우수상)

- 기간 : 2019.04.06 ~ 2019.06.18
- 언어 : Java
- 역할
    - 팀장
    - eye tracking 구현
    - Application 출시
    - Android Hardware 제어
- 관련 기술
    - `Java`, `Android`, `OpenCV`
- [Github Repository](https://github.com/wjrmffldrhrl/mobileteam)

### Movie Cloud

영화 리뷰 크롤링 데이터 시각화 시스템

> 교내 경진대회 출품작 (우수상)

- 기간 : 2019.10.19 ~ 2019.10.31
- 언어 : Python
- 역할
    - Web Crawling
    - Word Cloud 생성
    - 데이터 서버 전송
- 관련 기술
    - `Python`, `Web Crawling`, `Django`, `Konlpy`, `Word Cloud`
- [Github Repository](https://github.com/wjrmffldrhrl/Python-Movie-Recommendation)

### [TeamHash](http://ec2-3-17-16-176.us-east-2.compute.amazonaws.com:8080/)

대학생을 타겟으로 한 조별과제 협업 플랫폼

> 교내 경진대회 출품작

- 기간 : 2020.03.01 ~ 2020.07.01
- 언어 : Java, JavaScript
- 역할
    - Back-end 개발
    - DB 관리
    - AWS 배포 서버 관리
    - STOMP를 활용한 실시간 채팅서버 구현
    - REST API 파일 업로드 서버 구현
- 관련 기술
    - `Java`, `STOMP`, `Thymeleaf`, `Spring Boot`, `AWS`, `Maria DB`
- [Github Repository](https://github.com/kyu9341/TeamHash)

## 외부 프로젝트

### Mioo

AR기반 피팅룸 시스템

> 한이음 ICT 멘토링 사업 (입선)

- 기간 : 2019.05.18 ~ 2019.11.18
- 언어 : Python, C#
- 역할
    - 쇼핑몰 의류 정보 웹 크롤링
    - 크롤링 데이터 서버 송신
- 관련 기술
    - `Python`, `Web Crawling`, `Unity`
- [Github Repository](https://github.com/wjrmffldrhrl/mioo)

### 우렁케어

홈케어 IoT

> 2019 공개 SW 개발자 대회 출품작

- 기간 : 2019.09.17 ~ 2019.12.17
- 언어 : Python
- 역할
    - 팀장
    - 프로젝트 기획
    - 아두이노 센서 제어
    - 라즈베리파이 제어
    - 라즈베리파이와 아두이노 데이터 교환
- 관련 기술
    - `Python`, `Raspberry Pi`, `Arduino`, `C`, `IoT`, `Android`
- [Github Repository](https://github.com/wjrmffldrhrl/UleungCare)

### 마스크 분배 박스

마스크 분배 자동화 장치

> 2019 서울시 미세먼지 해커톤 (우수상)

- 기간 : 2019.06.26 ~ 2019.06.27
- 언어 : Python
- 역할
    - Raspberry PI 제어
    - 모터 제어
    - NFC모듈과의 통신 제어
    - php서버와의 통신제어
    - 심사위원들에게 완성된 프로토타입의 시연 및 제품 설명
- 관련 기술
    - `Python`, `IoT`, `NFC`, `PHP`, `Raspberry Pi`, `Arduino`
- [Github Repository](https://github.com/wjrmffldrhrl/hackathon)

### Armagyeddon

블록체인 기반 계모임 플랫폼

> 블록체인 시스템 엔지니어 양성과정 프로젝트 (진행중)

- 기간 : 2020.07.07 ~ 진행중
- 언어 : Java, Solidity, JavaScript
- 역할
    - 팀장
    - Spring Boot 기반 REST API Back-end 서버 구현
    - JWT를 활용한 유저 인증
    - React 기반 Front-end 구현
    - Solidity를 활용한 스마트컨트렉트 구현
    - AWS 및 Docker를 활용한 배포 서버 관리
- 관련 기술
    - `Java`, `JavaScript`, `Solidity`, `Ethereum`, `Spring Boot`, `Truffle`, `React`, `REST API`, `AWS`, `Docker`, `MySQL`, `Web3j`, `JWT`
- Github Repository
    - [Back-end](https://github.com/wjrmffldrhrl/armagyeddon-backend)
    - [Front-end](https://github.com/wjrmffldrhrl/armagyeddon-frontend)
    - [Contract](https://github.com/wjrmffldrhrl/armagyeddon-contract)

### Golden Block

블록체인 기반 산학협력 플랫폼

> 스터디 프로젝트 (진행중)

- 기간 : 2020.07.07 ~ 진행중
- 언어 : Java, Solidity, JavaScript
- 역할
    - Spring Boot 기반 REST API Back-end 서버 구현
    - React 기반 Front-end 구현
    - AWS 및 Docker를 활용한 배포 서버 관리
- 관련 기술
    - `Java`, `JavaScript`, `Solidity`, `Ethereum`, `Spring Boot`, `Truffle`, `React`, `REST API`, `AWS`, `Docker`, `MySQL`, `Web3j`, `JWT`
- [Github Repository](https://github.com/wjrmffldrhrl/goldenblock)
